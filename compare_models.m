% COMPARE MODELS 2 & 3 RESULTS - AUTOMATED VERSION
% ===============================================
% Automated comparison script to analyze differences between
% Model 2 (basic preprocessing) and Model 3 (ICA preprocessing) results
%
% FEATURES:
% - Interactive menu system for easy comparison selection
% - Batch mode to run all comparisons automatically
% - Handles single files, multiple files, and batch processing
% - Organized folder structure with predefined paths
% - Statistical significance testing
% - Enhanced metrics and visualizations
% - Support for manual processing comparison
%
% FOLDER STRUCTURE:
% The script expects files to be organized in the 'compare_models' folder:
%   compare_models/
%   ├── script/
%   │   ├── model2/     (script-processed Model 2 files)
%   │   └── model3/     (script-processed Model 3 files)
%   ├── manual/
%   │   ├── model2/     (manually-processed Model 2 files)
%   │   └── model3/     (manually-processed Model 3 files)
%   ├── results/        (comparison results - auto-generated)
%   └── logs/           (comparison logs - auto-generated)
%
% AUTOMATION MODES:
% 1. INTERACTIVE MODE (recommended - set run_mode = 'interactive'):
%    - Menu-driven selection
%    - Choose single or multiple comparisons
%    - Options include:
%      • Script vs Manual (Model 2)
%      • Script vs Manual (Model 3)  
%      • Model 2 vs Model 3 (Script-processed)
%      • Model 2 vs Model 3 (Manually-processed)
%      • All comparisons at once
%      • Custom selection
%
% 2. BATCH MODE (set run_mode = 'batch_all'):
%    - Automatically runs all possible comparisons
%    - No user interaction required
%    - Processes all available file pairs
%
% USAGE:
% Simply run the script! The default interactive mode will guide you through
% selecting which comparisons to perform. Results are automatically organized
% in timestamped folders for easy tracking.
%
% REQUIREMENTS:
% - EEGLAB must be installed and running
% - Matching files between groups (same base name)
% - Proper folder structure as described above

% === CONFIGURATION ===
% Automated comparison configuration
% Set run_mode to control how comparisons are executed:
% 'interactive' - Menu-driven selection (recommended)
% 'batch_all' - Run all possible comparisons automatically

run_mode = 'interactive';  % Change this to 'batch_all' if needed
...
% Setup paths based on new folder structure
script_dir = fileparts(mfilename('fullpath'));
compare_models_dir = fullfile(script_dir, 'compare_models');

% Verify compare_models directory exists
...
for i = 1:length(common_names)
    ...
    try
        ...
        m2_skewness = mean(skewness(EEG_m3.data, 1, 2));
        ...
        if EEG_m2.nbchan == EEG_m3.nbchan
            try
                ...
                avg_correlation = nanmean(correlations);
                min_correlation = nanmin(correlations);
